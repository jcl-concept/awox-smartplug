(function() {
	class AwoxSmartplug extends window.Extension {
	    constructor() {
	      	super('awox-smartplug');
			...
			console.log("Adding awox-smartplug addon to main menu");
			this.addMenuEntry('AWOX Smartplug');
	        ...
	        .then((text) => {
                ...
                // This is needed because the user might already be on the addon page and click on the menu item again. This helps to reload it.
	  		 	if( document.location.href.endsWith("extensions/awox-smartplug") ){
	  		  		this.show();
	  		  	}
	        })
	    ...
	    show() {
			console.log("awox-smartplug show called");
			//console.log("this.content:");
			//console.log(this.content);
            
            
			const main_view = document.getElementById('extension-awox-smartplug-view');
            ...
            // ADD button press
            document.getElementById('extension-awox-smartplug-add-item-button').addEventListener('click', (event) => {
            	console.log("first button clicked. Event: ", event);
                
                const new_name = document.getElementById('extension-awox-smartplug-add-item-name').value;
                const new_value = document.getElementById('extension-awox-smartplug-add-item-value').value;
				...
				window.API.postJson(
                    ...
                    if(body.state == true){
                        ...
                        document.getElementById('extension-awox-smartplug-add-item-name').value = "";
                        document.getElementById('extension-awox-smartplug-add-item-value').value = null;
					...
					console.log("awox-smartplug: connnection error after add new item button press: ", e);
			...
			document.getElementById('extension-awox-smartplug-title').addEventListener('click', (event) => {
            ...
            document.getElementById('extension-awox-smartplug-show-second-page-button').addEventListener('click', (event) => {
                console.log("clicked on + button");
                document.getElementById('extension-awox-smartplug-content-container').classList.add('extension-awox-smartplug-showing-second-page');
                
                // iPhones need this fix to make the back button lay on top of the main menu button
                document.getElementById('extension-awox-smartplug-view').style.zIndex = '3';
			});
            
            // Back button, shows main page
            document.getElementById('extension-awox-smartplug-back-button-container').addEventListener('click', (event) => {
                console.log("clicked on back button");
                document.getElementById('extension-awox-smartplug-content-container').classList.remove('extension-awox-smartplug-showing-second-page');
                
                // Undo the iphone fix, so that the main menu button is clickable again
                document.getElementById('extension-awox-smartplug-view').style.zIndex = 'auto';
            ...
            document.getElementById('extension-awox-smartplug-view').scrollTop = 0;
		...
		hide() {
			console.log("awox-smartplug hide called");
		}
			...
			try{
		        ...
		        window.API.postJson(
		          `/extensions/${this.id}/api/ajax`,
                    ...
                    document.getElementById('extension-awox-smartplug-loading').classList.add('extension-awox-smartplug-hidden');
                    ...
                    if(typeof body.debug != 'undefined'){
                        ...
                        if(body.debug == true){
                            ...
                            if(document.getElementById('extension-awox-smartplug-debug-warning') != null){
                                document.getElementById('extension-awox-smartplug-debug-warning').style.display = 'block';
                            }
                        }
                    }
                    ...
                    if(typeof body.a_number_setting != 'undefined'){
                        ...
                        document.getElementById('extension-awox-smartplug-number-setting-output').innerText = body.a_number_setting; // body['a_number_setting'] and body.a_number_setting are two ways of writing the same thing 
                    }
                    
                    // Show the value of the slider
                    document.getElementById('extension-awox-smartplug-slider-value-output').innerText = body.slider_value;
                    ...
                    if(typeof body.items_list != 'undefined'){
		  			...
		  			console.log("Error getting AwoxSmartplug init data: ", e);
		        });	
		...
		regenerate_items(items){
			...
			try {
                ...
                let list_el = document.getElementById('extension-awox-smartplug-main-items-list'); // list element
                if(list_el == null){
				...
				const original = document.getElementById('extension-awox-smartplug-original-item');
				...
				for( var item in items ){
                    ...
                    clone.querySelector(".extension-awox-smartplug-item-name").innerText = items[item].name; // The original and its clones use classnames to avoid having the same ID twice
                    clone.getElementsByClassName("extension-awox-smartplug-item-value")[0].innerText = items[item].value; // another way to do the exact same thing - select the element by its class name
                     
                    
                    // You could add a specific CSS class to an element depending, for example depending on some value
                    //clone.classList.add('extension-awox-smartplug-item-highlighted');   
                    

					// ADD DELETE BUTTON
					const delete_button = clone.querySelectorAll('.extension-awox-smartplug-item-delete-button')[0];
                        ...
                        if(confirm("Are you sure you want to delete this item?")){
    						...
    						window.API.postJson(
                                ...
                                if(body.state == true){
                                    ...
                                    event.target.closest(".extension-awox-smartplug-item").style.display = 'none'; // find the parent item
                                    // Remove the item form the list, or regenerate the entire list instead
                                    // parent4.removeChild(parent3);
                                }

    						}).catch((e) => {
    							console.log("awox-smartplug: error in delete items handler: ", e);
    						});
                        }
				  	});
